...
@st.cache_data(show_spinner="A comparar modelos de carga...", ttl=7200)
...
# ── GUARDAR ANÁLISES HRV NO DRIVE ─────────────────────────────────────────
st.divider()
st.subheader("💾 Guardar Análises HRV no Google Drive")

col_db1, col_db2 = st.columns(2)

with col_db1:
    if st.button("💾 Guardar HRV Diário", key="btn_save_hrv_daily"):
        ...
        try:
            if len(dw) > 0:
                data_w = dw.iloc[-1]
                hrv_val = float(data_w.get('hrv', 0) or data_w.get('HRV', 0) or 0)
                rhr_val = float(data_w.get('rhr', 0) or data_w.get('RHR', 0) or 0)
                sono_val = float(data_w.get('Horas de Sono', 0) or 0)
                stress_val = int(data_w.get('Stress Do dia', 0) or 0)
                ...
                resultado = save_hrv_daily_analysis(
                    ...
                    baseline_lag60=hrv_val,
                    notas="Guardado automaticamente do dashboard"
                )
                
                if resultado:
                    st.success("✅ HRV diário guardado no Drive!")
                else:
                    st.error("❌ Erro ao guardar HRV")
            else:
                st.warning("⚠️ Sem dados de wellness disponíveis")
        except Exception as e:
            st.error(f"❌ Erro: {str(e)}")

with col_db2:
    if st.button("💾 Guardar Padrões de Recuperação", key="btn_save_recovery"):
        from utils.drive_db_hrv_analyzer import save_recovery_patterns
        
        try:
            if len(dw) > 0:
                hrv_col = 'hrv' if 'hrv' in dw.columns else 'HRV'
                
                resultado = save_recovery_patterns(
                    periodo_inicio=str(dw['Data'].min()),
                    periodo_fim=str(dw['Data'].max()),
                    dias_analisados=len(dw),
                    hrv_stats={
                        'media': float(dw[hrv_col].mean()),
                        'std': float(dw[hrv_col].std()),
                        'min': float(dw[hrv_col].min()),
                        'max': float(dw[hrv_col].max())
                    },
                    rhr_media=float(dw['rhr'].mean()) if 'rhr' in dw.columns else float(dw['RHR'].mean()) if 'RHR' in dw.columns else 0,
                    sono_media=float(dw['Horas de Sono'].mean()) if 'Horas de Sono' in dw.columns else 0,
                    stress_media=float(dw['Stress Do dia'].mean()) if 'Stress Do dia' in dw.columns else 0,
                    padroes_identificados="Análise HRV automática",
                    recomendacoes="Continuar monitoramento",
                    notas="Guardado automaticamente do dashboard"
                )
                
                if resultado:
                    st.success("✅ Padrões guardados no Drive!")
                else:
                    st.error("❌ Erro ao guardar padrões")
            else:
                st.warning("⚠️ Sem dados de wellness disponíveis")
        except Exception as e:
            st.error(f"❌ Erro: {str(e)}")
